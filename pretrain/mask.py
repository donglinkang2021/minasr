...
def get_mask(x: torch.Tensor, mask_prob: float = 0.8, mask_length: int = 10, mask_pad: int = 0) -> torch.Tensor:
    """
    mask
    ...
    if T % mask_length != 0:
        x = x[:, :nt * mask_length, :]
    x = rearrange(x, 'b (nt ts) c -> b nt ts c', ts=mask_length)
    ...
    x.masked_fill_(mask, mask_pad)
    x = rearrange(x, 'b nt ts c -> b (nt ts) c')
    return x
